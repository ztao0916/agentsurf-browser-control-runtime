...
function Get-AgentSurfText {
  if ((Get-AgentSurfLanguage) -eq 'zh') {
    return @{
      ...
      NativeHostReplaceLocked = '无法替换 {0}，因为正在运行的 Native Host 锁定了该文件。请在浏览器的扩展页面中停用 AgentSurf，或关闭对应浏览器，等待几秒后重新执行安装命令。'
      ...
      Installed = '{0} Native Host 已安装，扩展 ID：{1}'
      Manifest = '清单文件：{0}'
      Config = '配置文件：{0}'
      Bridge = 'Bridge：ws://127.0.0.1:{0}'
      Reload = '请在 {0} 中重新加载 AgentSurf。移动项目目录或更换 Node 安装路径后，需要重新执行安装。'
      McpConfig = '将以下 {0} Server 加入你的 MCP 客户端配置：'
  ...
  return @{
    ...
    NativeHostReplaceLocked = 'Cannot replace {0} because the running native host keeps the file locked. Disable AgentSurf on the browser extensions page, or close that browser, wait a second, then run this command again.'
    ...
    Installed = '{0} Native Host installed for extension {1}'
    Manifest = 'Manifest: {0}'
    Config = 'Config:   {0}'
    Bridge = 'Bridge:   ws://127.0.0.1:{0}'
    Reload = 'Reload AgentSurf in {0}. Reinstall after moving the project or changing the Node installation path.'
    McpConfig = 'Add this {0} server to your MCP client configuration:'
